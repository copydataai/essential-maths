    ...
    const memoResult = createMemo<string>(() => {
        if (func() && domain()) {
            try {
                ...
                setError("please reload the page and please to sure to don't leave any character out of the equation")
            }
        }
        return ""
    })
        ...
        <main class="flex flex-col items-center space-y-4">
            <div>
                <label class="font-black">
                    f(
                    <input class="input input-primary input-xs w-8" type="number" onInput={getDomain} placeholder="x" value="3" />
                    )
                </label>
                <label class="font-black"> = </label>
                <input class="input input-secondary input-md" type="text" onInput={getFunction} placeholder="x^2+2x+4x" />
            </div>
            <Show when={error() != ""} fallback={
                <div class="card">
                    <div class="card-body ">
                        <h2 class="card-title">Result</h2>
                        <p class="text-accent font-black">{memoResult()}</p>
                    </div>
                </div>
            }>
                <div role="error" class="alert alert-error">
                    <img class="stroke-current shrink-0 h-6 w-6" src="/error.svg" alt="" />
                    <span >{error()}</span>
                </div>
            </Show>
        </main>
